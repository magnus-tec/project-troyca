...
<script>
 document.querySelector('#generarCuota').addEventListener('click', function(event) {
  ...
  const tasaAnual = parseFloat(document.querySelector('input[name="tem"]').value) / 100; // Dividir entre 100 para convertir a decimal
  ...
  if (modalidad === "mensual") {
    ...
    tasaInteres = tasaAnual / 12; // Tasa mensual, ya dividida entre 12
  } else {
    numPagos = cantidadCuotas;
    tasaInteres = tasaAnual / 365; // Tasa diaria, ya dividida entre 365
  }
  ...
  document.querySelector('input[name="ted"]').value = (tasaDiaria * 100).toFixed(4); // Mostrar tasa diaria en porcentaje
